...
perVar1 = tk.StringVar()
perVar2 = tk.StringVar()
perVar3 = tk.StringVar()
perVar4 = tk.StringVar()
perVar5 = tk.StringVar()
ohmVar1 = tk.StringVar()
ohmVar2 = tk.StringVar()
ohmVar3 = tk.StringVar()
ohmVar4 = tk.StringVar()
ohmVar5 = tk.StringVar()
...
def channelPage(channelNumber):
    ...
    plot1.plot(time,impedanceN, color = "red", zorder=1, marker="_")
    fig.gca()
...
def configureFunction(timeEnt, dateEnt):
    global configure_bytes, board_status, function_flag, passFlag
...
def calibrateInside(per, ohm):
    ...
    negative = []
    for i in range(5):
        calibdata[6+i] = chr(per[i])
        if(ohm[i]>=0):
            calibdata[11+i] = chr(ohm[i])
        else:
            negative.append(11+i)
    
    calibdata[16] = "K"
    
    s = ""
    print(calibdata)
    calibdata = bytes(s.join(calibdata), 'utf-8')
    calibdata = bytearray(calibdata)
    for i in negative:
        calibdata[i] = 256+ohm[i-11]
    calibdata = bytes(calibdata)
    
    client = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    command = b"Calibrate" # Read or Configure
    client.sendto(command, addr)
    send_recv = client.recvfrom(5)
    print(send_recv)
    client.sendto(calibdata, addr)
    send_recv = client.recvfrom(20)
    print(send_recv)
    client.sendto(b'Exit', addr)
    status_1 = client.recvfrom(12)
...
def calibrateFunction():
    global function_flag, passFlag
    print("calibrateFunction")
    if(passw==""):
        messagebox.showerror("Error", "Please fill in Password")
        return None
    try:
        per = []
        per.append(int(perVar1.get()))
        per.append(int(perVar2.get()))
        per.append(int(perVar3.get()))
        per.append(int(perVar4.get()))
        per.append(int(perVar5.get()))
        
        ohm = []
        ohm.append(int(ohmVar1.get()))
        ohm.append(int(ohmVar2.get()))
        ohm.append(int(ohmVar3.get()))
        ohm.append(int(ohmVar4.get()))
        ohm.append(int(ohmVar5.get()))
        print(per, ohm)
        for i in range(5):
            if(per[i]>200 or per[i]<0):
                messagebox.showerror("Error", "Percentage must be in range 0 to 200 %")
            if(ohm[i]<-100 or ohm[i]>100):
                messagebox.showerror("Error", "Offset must be in range -100 to 100 K")
    
        if(passFlag and not function_flag):
            stat = calibrateInside(per, ohm)
        else:
            sendPasswordAndVerify()
            stat = calibrateInside(per, ohm)
        function_flag = True
        ...
        if stat[0] == b"RECEIVED":
            messagebox.showinfo("Success!", "Calibration was successful.")
        else:
            messagebox.showerror("Error", "Couldn't calibrate the device, please retry.")
...
def calibarateSection(frame2):
    chnLabel = ttk.Label(frame2, text="Channel", background='#DBD9D5', font=("",9,"bold"), foreground="blue")
    chnLabel.grid(column=0, row=1, padx=5, sticky='S')
    
    perLabel = ttk.Label(frame2, text="Per", background='#DBD9D5', font=("",9,"bold"), foreground="black")
    perLabel.grid(column=1, row=1, sticky='S')
    
    perceLabel = ttk.Label(frame2, text="(%)", background='#DBD9D5', font=("",8,"bold"), foreground="black")
    perceLabel.grid(column=1, row=2, sticky='N')
    
    offLabel = ttk.Label(frame2, text="Offset", background='#DBD9D5', font=("",9,"bold"), foreground="black")
    offLabel.grid(column=2, row=1, sticky='S')

    offLabel1 = ttk.Label(frame2, text="(K-Ohm)", background='#DBD9D5', font=("",8,"bold"), foreground="black")
    offLabel1.grid(column=2, row=2, sticky='N')
    
    calibButton = tk.Button(frame2, text = 'Calibrate',  font=("",11,"bold"), background="#92D050", foreground="white", command = calibrateFunction)
    calibButton.grid(column=0, row=0, columnspan=3, padx=5, pady=5, sticky=tk.W + tk.E)
    
    for i in range(5):
        chnLabel = ttk.Label(frame2, text="CH{}".format(i), background='#DBD9D5', font=("",9,"bold"), foreground="black")
        chnLabel.grid(column=0, row=i+3, padx=5, pady=5, sticky='N')
    
    perEnt1=tk.Entry(frame2, width=5, textvariable=perVar1)
    perEnt1.grid(column=1, row=3, padx=5, pady=5, sticky='SW')
    
    perEnt2=tk.Entry(frame2, width=5, textvariable=perVar2)
    perEnt2.grid(column=1, row=4, padx=5, pady=5, sticky='SW')
    
    perEnt3=tk.Entry(frame2, width=5, textvariable=perVar3)
    perEnt3.grid(column=1, row=5, padx=5, pady=5, sticky='SW')
    
    perEnt4=tk.Entry(frame2, width=5, textvariable=perVar4)
    perEnt4.grid(column=1, row=6, padx=5, pady=5, sticky='SW')
    
    perEnt5=tk.Entry(frame2, width=5, textvariable=perVar5)
    perEnt5.grid(column=1, row=7, padx=5, pady=5, sticky='SW')
    
    offEnt1=tk.Entry(frame2, width=5, textvariable=ohmVar1)
    offEnt1.grid(column=2, row=3, padx=5, pady=5, sticky='SW')
    
    offEnt2=tk.Entry(frame2, width=5, textvariable=ohmVar2)
    offEnt2.grid(column=2, row=4, padx=5, pady=5, sticky='SW')
    
    offEnt3=tk.Entry(frame2, width=5, textvariable=ohmVar3)
    offEnt3.grid(column=2, row=5, padx=5, pady=5, sticky='SW')
    
    offEnt4=tk.Entry(frame2, width=5, textvariable=ohmVar4)
    offEnt4.grid(column=2, row=6, padx=5, pady=5, sticky='SW')
    
    offEnt5=tk.Entry(frame2, width=5, textvariable=ohmVar5)
    offEnt5.grid(column=2, row=7, padx=5, pady=5, sticky='SW')
...
frame1.grid(column=1, row=1, padx=5, pady=5, columnspan=3, rowspan=2, sticky='W')

frame2 = tk.LabelFrame(root, borderwidth=2, background='#DBD9D5')
frame2.grid(column=0, row=2, padx=5, pady=5, sticky='S')
...
passwordEnt=tk.Entry(passFrame, width=4, background="#BFBFBF", foreground="white", textvariable=password, show='*')
passwordEnt.insert(0, '----')
